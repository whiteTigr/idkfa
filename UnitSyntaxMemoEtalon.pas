...
uses
    ...
    Contnrs, Clipbrd, ExtCtrls, ComCtrls, Menus, ForthTC;
...
begin
    ...
    fLitReference     := '@';
    fLitDereference   := '^';
    fParseOptions     := [  poHasMLComment, poHasILComment,
                            poHasHexPrefix, poFloatValid, poHasReference,
                            poHasDereference, poHasILCompDir];
    ...
    //User Tokens
    fTokenStyles[tokImmediate].tsForeground  := $003F7F;
    fTokenStyles[tokImmediate].tsBackground  := clWhite;
    fTokenStyles[tokImmediate].tsStyle       := [fsBold];

    fTokenStyles[tokAlfaBet].tsForeground    := clBlack;
    fTokenStyles[tokAlfaBet].tsBackground    := clWhite;
    fTokenStyles[tokAlfaBet].tsStyle         := [];

    fTokenStyles[tokVar].tsForeground        := clBlack;
    fTokenStyles[tokVar].tsBackground        := clWhite;
    fTokenStyles[tokVar].tsStyle             := [];

//    fTokenStyles[tokDict].tsForeground       := RGB(100, 0, 0);
//    fTokenStyles[tokDict].tsBackground       := clWhite;
//    fTokenStyles[tokDict].tsStyle            := [];

    fTokenStyles[tokErrorDict].tsForeground  := clRed;
    fTokenStyles[tokErrorDict].tsBackground  := clWhite;
    fTokenStyles[tokErrorDict].tsStyle       := [fsBold, fsUnderline];
end;
        ...
        if Line[i] > ' ' then begin
            ...
            if InWord then begin
                ...
                tokMLCompDirBeg:
                    ...
                    begin
                        if not InLit then NotLongSet := true;
                        InLit := True;
                    end;
                    { Test for string end }
                    if InLit then begin
                    ...
                    if (Length(si) > 0) and (si[1] in ['-', '0'..'9']) then begin
...
begin
    ...
    fGutterWidth    := 32;
    fInsertMode     := True;
    fOptions        := [smoAutoGutterWidth,smoShowCursorPos,smoShowPageScroll,smoPanning];
    fStepDebugLine  := -1;
...
begin
    inherited;
    case Key of
        #32..#255 : if (not (smoReadOnly in fOptions))and(self.Lines.Count>0) then
...
begin
    ...
    with ACanvas do begin
        ...
        Font.Color := clBlack;
        Font.Style := [];
        TextOut(GR.Right - TextWidth('0') - TextWidth(lineNumber), GR.Top, lineNumber);
    end;
end;
