...
var
  fKf7Downloader: TfKf7Downloader;
...
procedure TfKf7Downloader.bCloseClick(Sender: TObject);
begin
  Close;
end;
...
var
  ...
  coreIndex: integer;
begin
  CoreIndex := fKf7Downloader.cbCore.ItemIndex;
  SendBufferPtr := 0;
...
procedure SetReset;
begin
  downloader.SendByte(SetResetCmd[fKf7Downloader.cbCore.ItemIndex]);
end;

procedure ClearReset;
begin
  downloader.SendByte(ClearResetCmd[fKf7Downloader.cbCore.ItemIndex]);
end;
...
begin
  ...
  fKf7Downloader.ProgressBar1.Show;
  gauge.Show;
  fKf7Downloader.labelStatus.Hide;
  fKf7Downloader.bCancel.Show;
  fKf7Downloader.bCancel.Enabled := true;
end;

procedure HideVisualElements;
begin
  fKf7Downloader.ProgressBar1.Hide;
  gauge.hide;
  fMain.Hide;
  fMain.Show;
  if fKf7Downloader.Visible then fKf7Downloader.Show;
  fKf7Downloader.bCancel.Hide;
  fKf7Downloader.bCancel.Enabled := false;
  fKf7Downloader.labelStatus.Show;
end;

procedure ReadSendCountPackage;
begin
  if not TryStrToInt(fKf7Downloader.eCountPackage.Text, SendCountPackage) then
  begin
...
begin
  if not TryStrToFloat(fKf7Downloader.eWaitingCoef.Text, SendWaitingCoef) then
  begin
...
procedure TfKf7Downloader.ProcessDownloadExecute(Sender: TObject);
var
  StopPtr: integer;
  time: real;
  f: TextFile;
...
procedure TfKf7Downloader.bCancelClick(Sender: TObject);
begin
  UserBreak := true;
end;

procedure TfKf7Downloader.bSendByteClick(Sender: TObject);
var
  downloadCom: TDownloaderCom absolute downloader;
begin
...
procedure TfKf7Downloader.bSelectFileClick(Sender: TObject);
begin
...
procedure TfKf7Downloader.bSendFileClick(Sender: TObject);
var
  Buf : byte;
  i : integer;
...
procedure TfKf7Downloader.bSaveToFileClick(Sender: TObject);
var
  i: integer;
  HowMany: integer;
  fSave: file of byte;
