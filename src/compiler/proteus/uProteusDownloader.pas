unit uProteusDownloader;
// todo: ужасный код. Нужен рефакторинг
...
type
  TfProteusDownloader = class(TForm)
    bClose: TBitBtn;
    cbComName: TComboBox;
...
var
  fProteusDownloader: TfProteusDownloader;
...
procedure TfProteusDownloader.bCloseClick(Sender: TObject);
begin
  Close;
end;
...
var
  ...
  coreIndex: integer;
begin
  CoreIndex := fProteusDownloader.cbCore.ItemIndex;
  SendBufferPtr := 0;
...
procedure SetReset;
begin
  downloader.SendByte(SetResetCmd[fProteusDownloader.cbCore.ItemIndex]);
end;

procedure ClearReset;
begin
  downloader.SendByte(ClearResetCmd[fProteusDownloader.cbCore.ItemIndex]);
end;
...
begin
  ...
  fProteusDownloader.ProgressBar1.Show;
  gauge.Show;
  fProteusDownloader.labelStatus.Hide;
  fProteusDownloader.bCancel.Show;
  fProteusDownloader.bCancel.Enabled := true;
end;

procedure HideVisualElements;
begin
  fProteusDownloader.ProgressBar1.Hide;
  gauge.hide;
  fMain.Hide;
  fMain.Show;
  if fProteusDownloader.Visible then fProteusDownloader.Show;
  fProteusDownloader.bCancel.Hide;
  fProteusDownloader.bCancel.Enabled := false;
  fProteusDownloader.labelStatus.Show;
end;

procedure ReadSendCountPackage;
begin
  if not TryStrToInt(fProteusDownloader.eCountPackage.Text, SendCountPackage) then
  begin
...
begin
  if not TryStrToFloat(fProteusDownloader.eWaitingCoef.Text, SendWaitingCoef) then
  begin
...
// todo: слишком большая процедура
procedure TfProteusDownloader.ProcessDownloadExecute(Sender: TObject);
var
  StopPtr: integer;
  time: real;
  f: TextFile;
...
procedure TfProteusDownloader.bCancelClick(Sender: TObject);
begin
  UserBreak := true;
end;

procedure TfProteusDownloader.bSendByteClick(Sender: TObject);
var
  downloadCom: TDownloaderCom absolute downloader;
begin
...
procedure TfProteusDownloader.bSelectFileClick(Sender: TObject);
begin
...
procedure TfProteusDownloader.bSendFileClick(Sender: TObject);
var
  Buf : byte;
  i : integer;
...
procedure TfProteusDownloader.bSaveToFileClick(Sender: TObject);
var
  i: integer;
  HowMany: integer;
  fSave: file of byte;
